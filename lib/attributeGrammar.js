...
module.exports = require('hot-cocoa').analyzer({
  ...
  '_puncuated-list': function(tree) {
    ...
    switch (tree[0].type) {
    case '\'' :
      result.push(types('identifier', 'quote', tree[0].position));
      break;
    case '`' :
      result.push(types('identifier', 'quasiquote', tree[0].position));
      break;
    case '~' :
      result.push(types('identifier', 'unquote', tree[0].position));
      break;
    }
