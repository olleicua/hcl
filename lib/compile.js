...
var validate_args = function(number, range, function_name, position) {
  if (range === undefined) {
    throw new Error(format('No argument range specified for `~~` at position ~~',
						   [function_name, position]));
  }
  if (typeof(range) === 'number') {
    if (number !== range) {
      throw new Error(format('Wrong number of arguments for `~~`: ~~ for ~~ at position ~~',
...
var compile = function(ast, path) {
  if (ast.type === 'list') {
    var function_reference = compile(ast[0], path);
    ...
    var args = _.map(ast.slice(1), function(x) {
      return is_lazy ? x : compile(x, path);
    });
    var format_options = {
      compile: (is_lazy ? (function(x) { return compile(x, path); }) : _.identity),
      path: path
    };
...
module.exports = function(asts, source, path) {
  source = source || '';
