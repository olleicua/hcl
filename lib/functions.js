...
var function_map = require('hot-cocoa').template_map({
  ...
  'for': function(args, options) {
    var loop_init, value_iterator, key_iterator, list_ref;
    ...
    if (args[0].length === 3) {
      ...
      if (_.isArray(args[0][0])) {
        ...
        loop_body = format('(function(~1~, ~2~) { ~4~; }).call(this, ~0~, ~3~[~0~])',
                           [dummy_iterator, key_iterator, value_iterator,
                            list_ref, loop_body]);
      } else {
        value_iterator = options.compile(args[0][0]);
        require_mutable(value_iterator);
        loop_body = format('(function(~1~) { ~3~; }).call(this, ~2~[~0~])',
                           [dummy_iterator, value_iterator, list_ref, loop_body]);
      }
      loop_init = format('~0~ = 0; ~0~ < ~1~.length; ~0~++',
                         [dummy_iterator, list_ref]);
    } else {
      throw new Error('Invalid loop initialization for `for`');
    }
  ...
  'times': function(args, options) {
    ...
    return format('(function() {for (~0~ = 0; ~0~ < ~1~; ~0~++) { ' +
