...
var function_map = require('hot-cocoa').template_map({
  ...
  // TODO: add object access forms (abstracted?)
  'set+': '(~~ += ~~)',
  'set-': '(~~ -= ~~)',
  'set*': '(~~ *= ~~)',
  'set/': '(~~ /= ~~)',
  ...
  'let': function(args, options) {
    ...
    var return_statement = unreturnable(args.slice(-1)[0]) ? '' : 'return';
    return format('(function(~~) {~~ ~~ ~~; }).call(this~~)',
                  [keys.join(', '), expressions, return_statement,
                   final_expression, values.join('')]);
  },
  '#': function(args, options) {
    ...
    return format('(function(~~) {~~ ~~ ~~ ~~; })',
                  [func_args, vars_init, expressions, return_statement, final_expression]);
  },
  // ?? are these necessary ??
  'quote': function(args) {
    return args.json();
