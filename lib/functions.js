...
var function_map = require('hot-cocoa').template_map({
  ...
  'for': function(args, options) {
    var loop_init, value_iterator, key_iterator;
    var iterator_init = '';
    ...
    if (args[0].length === 3) {
      ...
      if (_.isArray(args[0][0])) {
        ...
        key_iterator = format('_i~~_', [options.context.dummy_iterator]);
        options.context.dummy_iterator++;
        options.context.add_to_scope(key_iterator);
        value_iterator = options.compile(args[0][0]);
        require_mutable(value_iterator);
      }
      loop_init = format('~0~ = 0; ~0~ < ~1~.length; ~0~++',
                         [key_iterator, loop_init_args[1]]);
      iterator_init = format('var ~0~ = ~1~[~2~];',
                             [value_iterator, loop_init_args[1], key_iterator]);
    } else {
      throw new Error('Invalid loop initialization for `for`');
    }
