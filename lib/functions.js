...
var function_map = require('hot-cocoa').template_map({
  ...
  'begin': function(args, options) {
    ...
    return format('(function() {~~ ~~ ~~; }).call(this)',
                  [expressions, return_statement, final_expression]);
  },
  'if': '(~~ ? ~~ : ~~)',
  'when': function(args) {
	// Should this simply be (~~ && ~~) when args.length === 2 ??
    return format('(~~ && (function() { ~~; }).call(this))',
  ...
  'for': function(args, options) {
    var iterator_init = '';
    var loop_init_args = _.map(args[0], options.compile);
    if (args[0].length === 3) {
      var loop_init = loop_init_args.join('; ');
    } else if (args[0].length === 2) {
      var loop_init = format('var _i_ = 0; _i_ < ~1~.length; _i_++',
                             loop_init_args);
      iterator_init = format('var ~0~ = ~1~[_i_];', loop_init_args);
    } else {
  ...
  'times': function(args, options) {
    return format('for (~~) { ~~; }', [
      format('var ~0~ = 0; ~0~ < ~1~; ~0~++', _.map(args[0], options.compile)),
      _.map(args.slice(1), options.compile).join('; ')
    ]);
  },
  ...
  'attempt': function(args, options) {
    ...
    return format('~~ ~~ ~~', [try_block, catch_block, finally_block]);
  },
  ...
  'let': function(args, options) {
    ...
    var return_statement = unreturnable(args.slice(-1)[0]) ? '' : 'return';
    return format('(function(~~) {~~ ~~ ~~; }).call(this~~)',
                  [keys.join(', '), expressions, return_statement, final_expression, values.join('')]);
  },
  ...
  'compile': function(args, options) {
    file2js(options.path + JSON.parse(args[0]));
...
function_map.set_properties(['++', '--', 'not', 'quote', 'quasiquote', 'error',
                             'inherit', 'empty?', 'even?', 'odd?', 'nil?',
                             'boolean?', 'number?', 'string?', 'list?',
                             'object?', 'function?', 'integer?', '+1', '-1',
                             '*2', '/2', '^2', 'sqrt', 'string', 'number',
                             'integer', 'size', '=0', 'compile'],
                            { args: 1 });
...
// (i.e. special forms)
function_map.set_properties(['#', 'let', 'quote', 'cond', 'for', 'times', 'attempt', 'begin'], { lazy: true });
...
function_map.set_synonyms('!=', ['isnt?', 'neq?']);
